...
The hole-card contract now records the exact risk root cause (`ocr_hole_card_extraction_missing_or_unreliable`), the source column (`players.cards`), the affected strength features, and the required handling policy. Missing or invalid private cards must be flagged and routed; they must not be silently imputed as known private-card strength or treated as reliable zero-strength evidence. This keeps the current delivery unblocked while preventing final strategy-quality claims until reviewed hole-card coverage improves.

Outcome-field leakage is guarded by an explicit temporal contract. Fields such as `winner_positions`, `dealer_winner`, `dealer_pot`, `ending_stack`, `stack_delta`, and `pot_from_stacks` are post-hand or settlement-derived values: they may remain in raw CSV files for audit, reporting, and settlement analysis, but they are forbidden as training features, prediction request fields, and model artifact features. If any of these fields enter the decision-time feature path, the delivery verifier treats it as a production blocker. This boundary is generated at `reports\data_leakage_contract.json`, rendered at `reports\data_leakage_contract.md`, and exposed through `GET /data-leakage-contract.json`.
...
`actions.csv` is treated as an incomplete betting-context table, not as a fully instrumented decision log. The native table has `action` and `street`, but fields such as `amount`, `to_call`, `pot_before_action`, `min_raise`, `legal_actions`, and `action_order` are missing or must be reconstructed from pre-action state. The training pipeline derives leakage-safe betting context from frame-ordered actions, stack contribution events, and street-local commitment state; the target action's own amount and post-hand outcome fields are explicitly forbidden as prediction features. This is a model-quality risk, not a current delivery blocker, and it is tracked at `reports\actions_context_quality.json`, rendered at `reports\actions_context_quality.md`, and exposed through `GET /actions-context-quality.json`.

`stack_events.csv` is treated as source data, not as a sufficient policy-feature table. Raw stack changes must be converted into decision-time `pot`, `effective_stack`, `SPR`, `bet_size`, and pressure features before training or inference. The target action's own stack delta remains label/evaluation context and is forbidden as a prediction feature. This boundary is generated at `reports\stack_event_context_quality.json`, rendered at `reports\stack_event_context_quality.md`, and exposed through `GET /stack-event-context-quality.json`.
...
The approved current-delivery LLM scope is limited to `event_normalization` and `decision_context`. `candidate_ranking` remains a research-only baseline, while `real_policy_agent` and `fully_autonomous_poker_playing_llm_policy` are explicitly excluded from the current delivery and require separate stakeholder approval, simulation gates, and production hardening before they can be claimed.

The term `LLM-based agent` is treated as an umbrella term and must be qualified by role. The implemented roles are `event_normalization` and `decision_context`; `candidate_ranking` is tracked as a research baseline; `real_policy_agent` is outside the current delivery scope and cannot be claimed as implemented or production-approved without a separate simulation and stakeholder approval gate.

The claim validator `poker_agent.llm_role_boundary.validate_llm_agent_claim` blocks unqualified production claims. For example, `LLM-based agent is production-ready` fails unless the claim names an approved role and that role is explicitly production-approved by the boundary contract.

An experimental LLM policy adapter is also implemented for offline architecture evaluation. It builds the full in-context poker prompt, calls an injected LLM backend, parses strict JSON output, filters illegal actions, normalizes probabilities, applies a confidence threshold, and falls back deterministically when the output is invalid. This adapter is intentionally not served by `POST /predict`, is not production-approved, and does not affect the deployed strategy stack. Its contract is generated at `reports\llm_policy_experimental.json`, rendered at `reports\llm_policy_experimental.md`, and exposed through `GET /llm-policy-experimental.json`.

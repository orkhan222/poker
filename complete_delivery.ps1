...
$MlopsReport = Join-Path $ReportsDir "mlops_contract.json"
$ModelRegistryReport = Join-Path $ReportsDir "model_registry.json"
$ExperimentLog = Join-Path $ReportsDir "experiments.jsonl"
$DatasetVersionLog = Join-Path $ReportsDir "dataset_versions.jsonl"
$MonitoringReport = Join-Path $ReportsDir "monitoring_report.json"
$PredictionLog = Join-Path $ReportsDir "prediction_logs.jsonl"
$AuditTrail = Join-Path $ReportsDir "audit_trail.jsonl"
$SecurityReport = Join-Path $ReportsDir "security_report.json"
$FinalDeliverablesReport = Join-Path $ReportsDir "final_deliverables.json"
$FinalDeliverablesContract = Join-Path $ReportsDir "final_deliverables_contract.json"
$DeliveryReport = Join-Path $ReportsDir "delivery_report.md"
$ApiContractDocs = Join-Path $ProjectRoot "docs\API_CONTRACT.md"
...
Write-Host "6b/8 Emitting MLOps smoke contract..." -ForegroundColor Green
& $Python scripts\check_mlops_contract.py `
    --root $ProjectRoot `
    --dataset (Join-Path $ProjectRoot "sample_poker_log.jsonl") `
    --model $ModelOut `
    --out $MlopsReport `
    --registry-out $ModelRegistryReport `
    --experiment-log-out $ExperimentLog `
    --dataset-log-out $DatasetVersionLog `
    --smoke
if ($LASTEXITCODE -ne 0) {
    Write-Error "MLOps contract check failed."
}

Write-Host "6c/8 Emitting monitoring smoke contract..." -ForegroundColor Green
& $Python scripts\check_monitoring_contract.py `
    --root $ProjectRoot `
    --prediction-log-out $PredictionLog `
    --audit-trail-out $AuditTrail `
    --out $MonitoringReport `
    --smoke
if ($LASTEXITCODE -ne 0) {
    Write-Error "Monitoring contract check failed."
}

Write-Host "6d/8 Emitting security smoke contract..." -ForegroundColor Green
& $Python scripts\check_security_contract.py `
    --root $ProjectRoot `
    --out $SecurityReport `
    --smoke
if ($LASTEXITCODE -ne 0) {
    Write-Error "Security contract check failed."
}

Write-Host "6e/8 Preparing final deliverables..." -ForegroundColor Green
& $Python scripts\prepare_final_deliverables.py `
    --root $ProjectRoot `
    --out $FinalDeliverablesReport `
    --docs-out $ApiContractDocs `
    --delivery-report-out $DeliveryReport `
    --contract-out $FinalDeliverablesContract `
    --smoke
if ($LASTEXITCODE -ne 0) {
    Write-Error "Final deliverables check failed."
}
...
Write-Host "MLOps report: $MlopsReport"
Write-Host "Model registry: $ModelRegistryReport"
Write-Host "Monitoring report: $MonitoringReport"
Write-Host "Security report: $SecurityReport"
Write-Host "Final deliverables: $FinalDeliverablesReport"
Write-Host "API docs: $ApiContractDocs"
Write-Host "Delivery report: $DeliveryReport"
